...
# 🚀 PYROMOD EXTENDED DICTIONARY INITIALIZATION
try:
    import pyromod
    from pyromod.listen.listener_types import ListenerTypes
    # Pyromod registration lock fix
    if not hasattr(Client, 'listeners'):
        setattr(Client, 'listeners', {ListenerTypes.MESSAGE: {}, 'message': {}})
    elif 'message' not in Client.listeners:
        Client.listeners['message'] = {}
        Client.listeners[ListenerTypes.MESSAGE] = {}
except Exception as e:
    # Fallback to absolute dict registration if enum fails
    try:
        Client.listeners = {'message': {}, 'MESSAGE': {}}
    except:
        pass
