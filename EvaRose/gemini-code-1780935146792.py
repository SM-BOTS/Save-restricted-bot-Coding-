...
async def handle_private(client: Client, acc, message: Message, chatid, msgid: int):
    ...
    # Check database-backed dynamic custom thumbnail safely
    custom_thumb = await db.get_thumbnail(message.from_user.id)

    if "Document" == msg_type:
        try:
            ph_path = await acc.download_media(msg.document.thumbs[0].file_id) if not custom_thumb else custom_thumb
        except:
            ph_path = custom_thumb
        try:
            ...
            if ERROR_MESSAGE == True:
                await client.send_message(message.chat.id, f"Error: {e}", reply_to_message_id=message.id, parse_mode=enums.ParseMode.HTML)
        if ph_path != None and ph_path != custom_thumb:
            os.remove(ph_path)
            
    elif "Video" == msg_type:
        try:
            ph_path = await acc.download_media(msg.video.thumbs[0].file_id) if not custom_thumb else custom_thumb
        except:
            ph_path = custom_thumb
        try:
        ...
        try:
            ph_path = await acc.download_media(msg.audio.thumbs[0].file_id) if not custom_thumb else custom_thumb
        except:
            ph_path = custom_thumb
        try:
...
# ----------------------------------------------------
# ADVANCED MASTER CONTROL SETTINGS GRID
# ----------------------------------------------------
...
async def settings_callback(client, callback_query):
    ...
    try:
        ...
        await callback_query.answer("Settings current profile loaded.")

# --- LOGIN / LOGOUT OPERATIONS ---
@Client.on_callback_query(filters.regex("^req_login$"))
async def login_click_callback(client, callback_query):
    await callback_query.answer("Please use the global /login command to securely link your API credentials!", show_alert=True)
...
async def logout_click_callback(client, callback_query):
    ...
    if not session_exist:
        return await callback_query.answer("You are already Logged-Out!", show_alert=True)
    await db.rem_session(user_id)
    await callback_query.message.edit_text("🚪 **Session forcefully logged-out and cleared from DB.**")
...
async def set_dump_callback(client, callback_query):
    ...
    try:
        ...
        if response and response.text:
            ...
            await response.reply_text("Channel configured successfully.")
    except Exception as e:
...
async def remove_dump_callback(client, callback_query):
    ...
    await callback_query.message.edit_text("Backup channel detached.")

# --- CAPTION OPERATIONS ---
@Client.on_callback_query(filters.regex("^set_caption_info$"))
async def set_caption_callback(client, callback_query):
    await callback_query.message.delete()
    await client.send_message(chat_id=callback_query.from_user.id, text="Send your custom caption layout:")
    try:
        response = await client.listen(chat_id=callback_query.from_user.id, timeout=300)
        if response and response.text:
            custom_text = response.text.strip()
            batch_temp.CUSTOM_CAPTIONS[callback_query.from_user.id] = custom_text
            await response.reply_text("Custom description appended.")
    except Exception as e:
        await client.send_message(callback_query.from_user.id, f"Error: {e}")
...
async def remove_caption_callback(client, callback_query):
    user_id = callback_query.from_user.id
    if user_id in batch_temp.CUSTOM_CAPTIONS:
        del batch_temp.CUSTOM_CAPTIONS[user_id]
    await callback_query.message.edit_text("Dynamic caption wiped out.")

# --- THUMBNAIL OPERATIONS ---
@Client.on_callback_query(filters.regex("^set_thumb_info$"))
async def set_thumbnail_callback(client, callback_query):
    await callback_query.message.delete()
    await client.send_message(chat_id=callback_query.from_user.id, text="Please reply/send any JPEG/PNG image to save as thumbnail:")
    try:
        response = await client.listen(chat_id=callback_query.from_user.id, timeout=300)
        if response and response.photo:
            ...
            await response.reply_text("🟢 **Custom thumbnail locked successfully in DB!**")
        else:
            await client.send_message(callback_query.from_user.id, "❌ **Failed:** Send a proper media image file.")
    except Exception as e:
        await client.send_message(callback_query.from_user.id, f"Error: {e}")
...
async def remove_thumbnail_callback(client, callback_query):
    ...
    await callback_query.message.edit_text("🔴 **Custom static thumbnail removed. Default used.**")