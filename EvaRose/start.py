...
async def save(client: Client, message: Message):
    ...
    # --- 🔐 TOKEN VERIFICATION WALL START (YE RHA AAPKA TOKEN SYSTEM!) ---
...
async def handle_private(client: Client, acc, message: Message, chatid, msgid: int):
    ...
    if "Document" == msg_type:
        ...
        try:
            uploaded_msg = await client.send_photo(chat, file, caption=caption, reply_to_message_id=message.id, parse_mode=enums.ParseMode.HTML, progress=progress, progress_args=[message,"up"])
        except Exception as e:
            if ERROR_MESSAGE == True:
                await client.send_message(message.chat.id, f"Error: {e}", reply_to_message_id=message.id, parse_mode=enums.ParseMode.HTML)
...
# 🔘 Updates Callback Query Handler (CLEAN OVERHAUL)
