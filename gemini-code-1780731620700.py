...
def get_any_shorturl(user_id):
    try:
        ...
        loop.create_task(save_active_token(user_id, unique_token))
...
async def send_start(client: Client, message: Message):
    ...
    buttons = [
        ...
        [InlineKeyboardButton("🔍 sᴜᴘᴘᴏʀᴛ ɢʀᴏᴜᴘ", url="https://t.me/vj_bot_disscussion"),
         InlineKeyboardButton("🤖 ᴜᴘᴅᴀᴛê ᴄʜ2024_ᴄʜ2024", url="https://t.me/vj_bots")]
    ]
...
async def save(client: Client, message: Message):
    ...
    if batch_temp.USER_STATES.get(user_id) == "awaiting_phone_number":
        ...
        try:
            ...
            await message.reply_text(f"`/login {phone_number}`")
        return

    # --- 🔐 LOGIN WALL CHECK FIRST ---
    if "https://t.me/" in message.text and not message.text.startswith("/"):
        if LOGIN_SYSTEM == True:
            user_data = await db.get_session(user_id)
            if user_data is None:
                await message.reply("**For Downloading Restricted Content You Have To /login First. Go to ⚙️ Settings to Login.**")
                return

        # --- 🔐 TOKEN VERIFICATION WALL (LOGIN KE BAAD HE TRIGGER HOGI) ---
    ...
    if "https://t.me/" in message.text:
        ...
        if LOGIN_SYSTEM == True:
            ...
            try:
                ...
                return await message.reply("**Your Login Session Expired. So /logout First Then Login Again from Settings Menu.**")
        else:
            if TechVJUser is None:
...
async def handle_private(client: Client, acc, message: Message, chatid, msgid: int):
    ...
    if "Document" == msg_type:
        ...
        try: uploaded_msg = await client.send_photo(chat, file, caption=caption, reply_to_message_id=message.id, parse_mode=enums.ParseMode.HTML, progress=progress, progress_args=[message,"up"])
        except Exception as e:
            if ERROR_MESSAGE == True: await client.send_message(message.chat.id, f"Error: {e}", reply_to_message_id=message.id, parse_mode=enums.ParseMode.HTML)
    ...
    if uploaded_msg:
        ...
        if user_dump:
            try: await uploaded_msg.copy(int(user_dump))
            except: pass
...
# 🔘 Updates Callback Query Handler (WAPAS LOGIN BUTTONS JOD DIYE HAIN)
...
async def callback_handler(client, query: CallbackQuery):
    ...
    if query.data == "settings":
        ...
        # 🔥 ROW 1 me Login aur Logout wale dono buttons perfectly jor diye hain!
    ...
    # 🔑 LOGIN VIA DIRECT COMMAND
    elif query.data == "btn_login":
        await query.answer()
        batch_temp.USER_STATES[user_id] = None
        await client.send_message(chat_id=query.message.chat.id, text="/login")
        return

    # 🚪 LOGOUT VIA DIRECT COMMAND
    elif query.data == "btn_logout":
        await query.answer()
        batch_temp.USER_STATES[user_id] = None
        await client.send_message(chat_id=query.message.chat.id, text="/logout")
        return